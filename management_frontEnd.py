...
class Home:
    ...
    def __init__(self, root):
        ...
        self.btnThuNhap = Button(DataFrame, text="$$$\nTổng thu nhập", font=('arial', 25, 'bold'), bg="#2eb82e", fg="Ghost White", height=5, width=13)
...
class CTHoaDon:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Database Management Systems")
        self.root.geometry("785x695+500+65")
        self.root.config(bg="#b3ffb3")
        ...
        TitFrame = Frame(MainFrame, bd=2, padx=54, pady=8, bg="#ccffff", relief=RIDGE)
        TitFrame.pack(side=TOP, pady=5)
        self.lblTit = Label(TitFrame, font=('arial', 47, 'bold'), text="Chi tiết Hóa đơn", fg="green", bg="#ccffff")
        self.lblTit.grid(row=0, columnspan=4)
        ...
        self.lblHangMuaID.grid(row=0, column=0, sticky=W)
        self.txtHangMuaID = Entry(DataFrameLEFT, font=('arial', 14, 'bold'), textvariable=hangMuaID, bg='Ghost white', width=50)
        self.txtHangMuaID.grid(row=0, column=1)

        self.lblTenHangMua = Label(DataFrameLEFT, font=('arial', 14, 'bold'), text="Tên hàng:", padx=2, pady=2, bg='Ghost white')
        self.lblTenHangMua.grid(row=1, column=0, sticky=W)
        self.txtTenHangMua = Entry(DataFrameLEFT, font=('arial', 14, 'bold'), textvariable=tenHangMua, bg='Ghost white', width=50)
        self.txtTenHangMua.grid(row=1, column=1)

        self.lblSLMua = Label(DataFrameLEFT, font=('arial', 14, 'bold'), text="Số lượng mua:", padx=2, pady=2, bg='Ghost white')
        self.lblSLMua.grid(row=2, column=0, sticky=W)
        self.txtSLMua = Entry(DataFrameLEFT, font=('arial', 14, 'bold'), textvariable=SLMua, bg='Ghost white', width=50)
        self.txtSLMua.grid(row=2, column=1)

        self.lblDonGia = Label(DataFrameLEFT, font=('arial', 14, 'bold'), text="Đơn giá:", padx=2, pady=2, bg='Ghost white')
        self.lblDonGia.grid(row=3, column=0, sticky=W)
        self.txtDonGia = Entry(DataFrameLEFT, font=('arial', 14, 'bold'), textvariable=donGiaBanCTHD, bg='Ghost white', width=50)
        self.txtDonGia.grid(row=3, column=1)

        txt1 = Label(DataFrameLEFT, bg='Ghost white')
        txt1.grid(row=12, columnspan=2, pady=5)
        ...
        lblHangMuaID = Label(ListFrame, font=('Arial', 11, 'bold'), text='Mã hàng', padx=35, pady=2, bg='Ghost white')
        lblHangMuaID.place(x=85, y=1)

        lblTenHangMua = Label(ListFrame, font=('Arial', 11, 'bold'), text='Tên hàng', padx=70, pady=2, bg='Ghost white')
        lblTenHangMua.place(x=220, y=1)

        lblSLMua = Label(ListFrame, font=('Arial', 11, 'bold'), text='Số lượng', padx=20, pady=2, bg='Ghost white')
        lblSLMua.place(x=432, y=1)

        lblDonGia = Label(ListFrame, font=('Arial', 11, 'bold'), text='Đơn giá', padx=37, pady=2, bg='Ghost white')
        lblDonGia.place(x=544, y=1)

        scrollbar = Scrollbar(ListFrame)
        scrollbar.grid(row=1, column=1, sticky= 'ns')

        khachList = Listbox(ListFrame, width=74, height=9, font=('arial', 12, 'bold'), yscrollcommand=scrollbar.set)
        khachList.bind('<<ListboxSelect>>')
        khachList.grid(row=1, column=0, padx=8)
        scrollbar.config(command=khachList.yview)

        #================================================ButtonFrame=================================================================

        self.btnAddData = Button(ButtonFrame, text='Thêm', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=5)
        self.btnAddData.grid(row=0, column=0)

        self.btnDisplay = Button(ButtonFrame, text='Hiển thị', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=7)
        self.btnDisplay.grid(row=0, column=1)

        self.btnClearData = Button(ButtonFrame, text='Dọn dẹp', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=7)
        self.btnClearData.grid(row=0, column=2)

        self.btnDeleteData = Button(ButtonFrame, text='Xóa', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=5)
        self.btnDeleteData.grid(row=0, column=3)

        self.btnSearchData = Button(ButtonFrame, text='Tìm kiếm', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=7)
        self.btnSearchData.grid(row=0, column=4)

        self.btnUpdateData = Button(ButtonFrame, text='Cập nhật', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=7)
        self.btnUpdateData.grid(row=0, column=5)

        self.btnComeBack = Button(ButtonFrame, text='Quay lại', font=('Arial', 12, 'bold'), height=1, width=8, bd=2, padx=5, command=HoaDonFn)
        self.btnComeBack.grid(row=0, column=6)
...
class HoaDon:
    ...
    def __init__(self, root):
        ...
        self.lblCTHD = Label(DataFrameLEFT, font=('arial', 14, 'bold'), text="Chi tiết hóa đơn:", padx=2, pady=9, bg='Ghost white')
        self.lblCTHD.grid(row=2, column=0, sticky=W)
        self.btnCTHD = Button(DataFrameLEFT, text='Chi tiết hóa đơn', font=('Arial', 12, 'bold'), height=1, width=15, bd=2, padx=5, command=CTHDFn)
        self.btnCTHD.grid(row=2, column=1)

        self.lblTongTriGia = Label(DataFrameLEFT, font=('arial', 14, 'bold'), text="Tổng trị Giá:", padx=2, pady=2, bg='Ghost white')
        self.lblTongTriGia.grid(row=3, column=0, sticky=W)
        self.txtTongTriGia = Entry(DataFrameLEFT, font=('arial', 14, 'bold'), textvariable=TongTriGia, bg='Ghost white', width=35)
        self.txtTongTriGia.grid(row=3, column=1)

        
        txt1 = Label(DataFrameLEFT, bg='Ghost white')
        txt1.grid(row=5, columnspan=2, pady=99)
        #===============================================display==================================================================
